#!/bin/bash
#Working on docker authentication for push and pull
...
if ! sudo bash -c "cat <<EOL > /etc/docker/daemon.json
{
  ...
  "insecure-registries": ["localhost:5000", "saranya8.mnsp.co.in"],
  "storage-driver": "overlay2",
  "tls": true,
  "tlscacert": "/etc/letsencrypt/live/saranya8.mnsp.co.in/fullchain.pem",
  "tlscert": "/etc/letsencrypt/live/saranya8.mnsp.co.in/fullchain.pem",
  "tlskey": "/etc/letsencrypt/live/saranya8.mnsp.co.in/privkey.pem"
}
...
server {
    ...
    server {
        if ($host = www.saranya8.mnsp.co.in){
            return 301 https://$host$request_uri;
        }
        if ($host = saranya8.mnsp.co.in){
            return 301 https://$host$request_uri;
        }

        listen 80;
        server_name saranya8.mnsp.co.in www.saranya8.mnsp.co.in;
        return 301 https://saranya8.mnsp.co.in$request_uri;
    }
}
